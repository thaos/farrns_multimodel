...
cmip5_prefix <- paste0(sub(pattern = "(.*)\\..*$", replacement = "\\1", cmip5_rds), "_KLweight_only2models")
cmip6_rds <- "tas_cmip6_augustavg.rds"
hadcrut_nc <- "tas_hadcrut_augustavg.nc"
p12outputs_rds <- "p12_augustavg_cmip56_KLweight.rds"
...
tas_cmip5 <- subset(tas_cmip5, grepl("r1i1p1", run))
...
# lmodels <-  lmodels[sample.int(length(lmodels), 5)]
...
lp12_pergridpoint <- lapply(inotna_gridpoint, function(igridpoint){
  ...
  p12_multimodel <- compute_allchain_KLweight(lmodels, tas_cmip5, kernel, bandwidth)
})
...
lweight_paris <- lp12_paris$lweight
weight_paris_df <- data.frame(
  model = factor(lmodels,  levels=lmodels[order(lweight_paris, decreasing = TRUE)]),
  weight = lweight_paris
)
pdf(paste0(cmip5_prefix,"_KLweight.pdf"), width = 20/2.54, height = 20/2.54)
p <- ggplot(data = weight_paris_df, aes(x = model, y = weight, fill = grepl("(CM6)", model))) +
...
lweight_allgridpoints <- mapply(
  function(x, igridpoint) {
    data.frame(institute = linstitutes, model = lmodels, weight = x$lweight, gridpoint = as.numeric(igridpoint))
  }, x = lp12_pergridpoint, igridpoint = inotna_gridpoint,
  SIMPLIFY = FALSE
) %>% do.call(rbind, .)
lweight_allgridpoints[, "gridpoint"] <- apply(lonlat_df[lweight_allgridpoints[, "gridpoint"], ], 1, function(x) paste0("[", x[1], ";", x[2], "]"))
lweight_allgridpoints <- within(
  lweight_allgridpoints, 
  gridpoint <- factor(gridpoint,  levels = unique(gridpoint)),
  model <- factor(model,  levels = lmodels[order(linstitutes)])
)
pdf(paste0(cmip5_prefix,"_lweight_allgripoints.pdf"), width = 20/2.54, height = 25/2.54)
p <- ggplot(lweight_allgridpoints) +
  geom_raster(aes(x = model, y = gridpoint, fill = weight)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_fill_gradientn(colours = rev(magma(5))) + 
  # xlim(1850, 2100) + ggtitle("p12(t), counterfactual = historical[1850-1900]")
  ggtitle(paste0("Model weights for each location"))
plot(p)
dev.off()

iter_KLfixpoint(
  weights = lp12_paris$lweight,
  q1 = lp12_paris$optimpars["q1m", ],
  V = lp12_paris$optimpars["Vm", ],
  I = lp12_paris$optimpars["Im", ]
)
