cmip5_rds <- "tas_cmip5_augustavg.rds"
cmip6_rds <- "tas_cmip6_augustavg.rds"
obs_rds <- "tas_hadcrut_paris.rds"
obs_lonlat <- c(2.5, 47.5)
obs_varname <- "tas"
p12obs_rds <- "p12_hadcrut_paris.rds"
p12outputs_rds <- "p12_augustavg_cmip56_experts_onerun.rds"
cmip5_prefix <- "tas_cmip5_augustavg_onerun_experts"
varname_inplot <- "tas, august mean"
unit_scaling <- identity
varunit <- "K"
city <- "paris"

